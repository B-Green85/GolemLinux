...
const fn is_authorized(ctx: CallerContext, op: Operation) -> bool {
    ...
    match (ctx, op) {
        ...
        // ReadAudit + Configure: Safe Mode only. This is the architectural
        // commitment that "Sentinel can be configured" is only true in the
        // agent-free recovery environment.
        (SafeMode, ReadAudit) => true,
        (_, ReadAudit) => false,
        (SafeMode, Configure) => true,
        (_, Configure) => false,
