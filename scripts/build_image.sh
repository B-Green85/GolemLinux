#!/usr/bin/env bash
#
# build_image.sh — Golem Linux Phase 3, Agent 1 (Boot Image Builder)
#
# Builds a bootable UEFI disk image (golem.img) holding the compiled kernel at
# the firmware fallback boot path  \EFI\BOOT\BOOTX64.EFI.
#
#   1. Create a 64 MiB raw disk image.
#   2. Lay down a GPT with a single FAT32 EFI System Partition.
#   3. Create EFI/BOOT/ and copy the kernel to BOOTX64.EFI.
...
#     byte-identical golem.img with the same SHA256. (Needs python3; if it is
#     unavailable the image is still valid and bootable, just not bit-reproducible.)
#
set -euo pipefail

# --- configuration ----------------------------------------------------------
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
KERNEL="$REPO_ROOT/target/x86_64-unknown-none/debug/gkern"
IMG="$REPO_ROOT/golem.img"
VOL_NAME="GOLEM"
IMG_SIZE_MB=64
...
# --- preflight --------------------------------------------------------------
if [[ ! -f "$KERNEL" ]]; then
  err "kernel binary not found: $KERNEL"
  err "build the kernel first (cargo build --target x86_64-unknown-none)"
  exit 1
fi
...
PART="${DEV}s1"
# Resolve the real mount point (handles names like "/Volumes/GOLEM 1").
...
log "creating EFI/BOOT/ and installing kernel as BOOTX64.EFI"
